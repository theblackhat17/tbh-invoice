  ...
  try {
    ...
    return new Promise<Response>((resolve) => {
      ...
      facture.prestations.forEach((p, i) => {
        ...
        doc.rect(col1, currentY - 5, 500, 25).fillAndStroke(bgColor, "#e0e0e0");

        doc
          .fillColor("#000000")
          .text(p.description, col1 + 5, currentY, { width: 280 })
          .text(p.quantite.toString(), col2 + 5, currentY)
          .text(`${p.prixUnit.toFixed(2)} €`, col3 + 5, currentY)
          .text(`${(p.quantite * p.prixUnit).toFixed(2)} €`, col4 + 5, currentY);
      ...
      doc.rect(col3 - 10, currentY, 162, 30).fillAndStroke("#3b82f6", "#3b82f6");
      doc
        .fillColor("#ffffff")
        .text("Total HT :", col3, currentY + 8)
        .text(`${facture.totalHT.toFixed(2)} €`, col4, currentY + 8, { align: "right" });
