...
export default function EditDevisPage() {
  ...
  const [devisNumero, setDevisNumero] = useState<string | null>(null);
  ...
  // Load clients + quote data
  useEffect(() => {
    if (!id) return;

    (async () => {
      try {
        const [rClients, rDevis] = await Promise.all([
          fetch('/api/clients'),
          fetch(`/api/devis?id=${id}`),
        ]);

        const dataClients = await rClients.json();
        const dataDevis = await rDevis.json();

        setClients(Array.isArray(dataClients) ? dataClients : []);

        setDevisNumero(dataDevis.numero ?? null);

        setForm({
          date:
            (dataDevis.date ?? '').slice(0, 10) ||
            new Date().toISOString().split('T')[0],
          clientId: dataDevis.clientId ?? '',
        });

        if (
          Array.isArray(dataDevis.prestations) &&
          dataDevis.prestations.length > 0
        ) {
          setPrestations(
            dataDevis.prestations.map((p: any) => ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/devis?id=${id}`, {
        method: 'PUT',
  ...
  return (
    <div className="py-10 max-w-5xl mx-auto">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Ajustez les informations et enregistrez pour mettre à jour le devis.
      </p>
